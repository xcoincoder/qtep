...
#include <qt/qtepversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    qtepVersionChecker = new QtepVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Qtep address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and qtep: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Qtep addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Qtep addresses"));
    ...
    openAction->setStatusTip(tr("Open a qtep: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Qtep command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && qtepVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(QTEP_RELEASES, QTEP_RELEASES);
            QString message(tr("New version of Qtep wallet is available on the Qtep source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Qtep network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
