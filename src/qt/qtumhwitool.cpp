...
class QtepHwiToolPriv
{
public:
    QtepHwiToolPriv(QObject *parent)
    {
...
QtepHwiTool::QtepHwiTool(QObject *parent) : QObject(parent)
{
    d = new QtepHwiToolPriv(this);
}

QtepHwiTool::~QtepHwiTool()
{
    delete d;
}

bool QtepHwiTool::enumerate(QList<HWDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    devices.clear();
    std::vector<LedgerDevice> vecDevices;
    if(QtepLedger::instance().enumerate(vecDevices, stake))
    {
    ...
    {
        d->strError = QString::fromStdString(QtepLedger::instance().errorMessage());
    }

    return devices.size() > 0;
}

bool QtepHwiTool::isConnected(const QString &fingerprint, bool stake)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    bool ret = QtepLedger::instance().isConnected(strFingerprint, stake);
    if(!ret) d->strError = QString::fromStdString(QtepLedger::instance().errorMessage());
    return ret;
}

bool QtepHwiTool::getKeyPool(const QString &fingerprint, int type, const QString& path, bool internal, QString &desc)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = QtepLedger::instance().getKeyPool(strFingerprint, type, strPath, internal, d->from, d->to, strDesc);
...
bool QtepHwiTool::getKeyPool(const QString &fingerprint, int type, const QString &path, QStringList &descs)
{
    LOCK(cs_ledger);
    bool ret = true;
    QString desc;
...
bool QtepHwiTool::getKeyPoolPKH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::LEGACY, path, descs);
}

bool QtepHwiTool::getKeyPoolP2SH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::P2SH_SEGWIT, path, descs);
}

bool QtepHwiTool::getKeyPoolBech32(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::BECH32, path, descs);
}

bool QtepHwiTool::signTx(const QString &fingerprint, QString &psbt)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    std::string strPsbt = psbt.toStdString();
    bool ret = QtepLedger::instance().signTx(strFingerprint, strPsbt);
    psbt = QString::fromStdString(strPsbt);
    if(!ret) d->strError = QString::fromStdString(QtepLedger::instance().errorMessage());
    return ret;
}

bool QtepHwiTool::signMessage(const QString &fingerprint, const QString &message, const QString &path, QString &signature)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = QtepLedger::instance().signMessage(strFingerprint, strMessage, strPath, strSignature);
    signature = QString::fromStdString(strSignature);
    if(!ret) d->strError = QString::fromStdString(QtepLedger::instance().errorMessage());
    return ret;
}

bool QtepHwiTool::signDelegate(const QString &fingerprint, QString &psbt)
{
...
QString QtepHwiTool::errorMessage()
{
...
bool QtepHwiTool::isStarted()
{
    return d->fStarted;
}

void QtepHwiTool::wait()
{
    if(d->fStarted)
    {
...
bool QtepHwiTool::rescanBlockchain(int startHeight, int stopHeight)
{
    if(!d->model) return false;
...
bool QtepHwiTool::importAddresses(const QString &desc)
{
    if(!d->model) return false;
...
bool QtepHwiTool::importMulti(const QStringList &descs)
{
    bool ret = true;
...
bool QtepHwiTool::finalizePsbt(const QString &psbt, QString &hexTx, bool &complete)
{
    if(!d->model) return false;
...
bool QtepHwiTool::sendRawTransaction(const QString &hexTx, QVariantMap& variantMap)
{
    if(!d->model) return false;
...
bool QtepHwiTool::decodePsbt(const QString &psbt, QString &decoded)
{
    if(!d->model) return false;
...
void QtepHwiTool::setModel(WalletModel *model)
{
    d->model = model;
}

bool QtepHwiTool::execRPC(ExecRPCCommand *cmd, const QMap<QString, QString> &lstParams, QVariant &result, QString &resultJson)
{
    d->strError.clear();
...
void QtepHwiTool::addError(const QString &error)
{
    if(d->strError != "")
        d->strError += "\n";
    d->strError += error;
...
QString QtepHwiTool::derivationPathPKH()
{
    std::string path = QtepLedger::instance().derivationPath((int)OutputType::LEGACY);
    return QString::fromStdString(path);
}

QString QtepHwiTool::derivationPathP2SH()
{
    std::string path = QtepLedger::instance().derivationPath((int)OutputType::P2SH_SEGWIT);
    return QString::fromStdString(path);
}

QString QtepHwiTool::derivationPathBech32()
{
    std::string path = QtepLedger::instance().derivationPath((int)OutputType::BECH32);
    return QString::fromStdString(path);
}
