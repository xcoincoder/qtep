...
using namespace QtepLedger_NS;

class QtepLedgerPriv
{
public:
    QtepLedgerPriv()
    {
        ...
        if(!toolExists)
        {
            LogPrintf("QtepLedger(): HWI tool not found %s\n", toolPath);
        }
    }
...
QtepLedger::QtepLedger():
    d(0)
{
    d = new QtepLedgerPriv();
}
...
QtepLedger &QtepLedger::instance()
{
    static QtepLedger device;
    return device;
}

bool QtepLedger::signCoinStake(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
...
bool QtepLedger::signBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &vchSig)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool QtepLedger::isConnected(const std::string &fingerprint, bool stake)
{
...
bool QtepLedger::enumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool QtepLedger::signTx(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool QtepLedger::signMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &signature)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool QtepLedger::getKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &desc)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
std::string QtepLedger::errorMessage()
{
    LOCK(cs_ledger);
...
bool QtepLedger::toolExists()
{
    return d->toolExists;
}

bool QtepLedger::isStarted()
{
    return d->fStarted;
}

void QtepLedger::wait()
{
    if(d->fStarted)
    {
...
bool QtepLedger::beginSignTx(const std::string &fingerprint, std::string &psbt)
{
...
bool QtepLedger::endSignTx(const std::string &, std::string &psbt)
{
...
bool QtepLedger::beginSignBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool QtepLedger::endSignBlockHeader(const std::string &, const std::string &, const std::string &, std::vector<unsigned char> &vchSig)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool QtepLedger::beginEnumerate(std::vector<LedgerDevice> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
    arguments << "enumerate";
...
bool QtepLedger::endEnumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    ...
    for(size_t i = 0; i < jsonDevices.size(); i++)
    {
        ...
        bool isStakeApp = device.app_name == "Qtep Stake" || device.app_name == "Qtep Stake Test";
        if(isStakeApp == stake)
        {
...
bool QtepLedger::beginSignMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool QtepLedger::endSignMessage(const std::string &, const std::string &, const std::string &, std::string &signature)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool QtepLedger::beginGetKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &)
{
    // Get the output type
    std::string descType;
    switch (type) {
...
bool QtepLedger::endGetKeyPool(const std::string &, int, const std::string& , bool, int, int, std::string &desc)
{
    // Decode command line results
...
std::string QtepLedger::derivationPath(int type)
{
