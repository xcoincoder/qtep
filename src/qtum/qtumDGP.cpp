...
void QtepDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool QtepDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule QtepDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t QtepDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t QtepDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t QtepDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t QtepDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool QtepDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void QtepDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void QtepDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void QtepDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void QtepDGP::createParamsInstance(){
...
dev::Address QtepDGP::getAddressForBlock(unsigned int blockHeight){
...
void QtepDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void QtepDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void QtepDGP::parseStorageOneUint64(uint64_t& value){
...
void QtepDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule QtepDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
