...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // qtep
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // qtep

    if(blockindex->IsProofOfStake()){
        result.pushKV("prevoutStakeHash", blockindex->prevoutStake.hash.GetHex()); // qtep
        result.pushKV("prevoutStakeVoutN", (int64_t)blockindex->prevoutStake.n); // qtep
    }
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // qtep
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // qtep
...
static std::vector<RPCResult> MempoolEntryDescription() { return {
    ...
    RPCResult{RPCResult::Type::NUM, "size", "(DEPRECATED) same as vsize. Only returned if qtepd is started with -deprecatedrpc=size\n"
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationinfoforaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The qtep address string"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
    ...
    if(!qtepDelegation.GetDelegation(address, delegation)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegation");
    }
    bool verified = qtepDelegation.VerifyDelegation(address, delegation);
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationsforstaker",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The qtep address string for staker"},
                },
               RPCResult{
            RPCResult::Type::ARR, "", "",
                {
    ...
    if(!qtepDelegation.FilterDelegationEvents(events, filter)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegations for staker");
    }
    std::map<uint160, Delegation> delegations = qtepDelegation.DelegationsFromEvents(events);
                        ...
                        {RPCResult::Type::OBJ, "scriptPubKey", "",
                            {
                                ...
                                {RPCResult::Type::ARR, "addresses", "array of qtep addresses",
                                    {{RPCResult::Type::STR, "address", "qtep address"}}},
                            }},
...
static UniValue qrc20balanceof(const JSONRPCRequest& request)
{
            RPCHelpMan{"qrc20balanceof",
                ...
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address"},
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO,  "The qtep address to check token balance"},
                },
                RPCResult{
...
static UniValue qrc20allowance(const JSONRPCRequest& request)
{
            RPCHelpMan{"qrc20allowance",
                ...
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address"},
                    {"addressFrom", RPCArg::Type::STR, RPCArg::Optional::NO,  "The qtep address of the account owning tokens"},
                    {"addressTo", RPCArg::Type::STR, RPCArg::Optional::NO,  "The qtep address of the account able to transfer the tokens"},
                },
                RPCResult{
...
static UniValue qrc20listtransactions(const JSONRPCRequest& request)
{
            RPCHelpMan{"qrc20listtransactions",
                ...
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address."},
                    {"addresss", RPCArg::Type::STR, RPCArg::Optional::NO,  "The qtep address to get history for."},
            ...
            RPCResult::Type::ARR, "", "",
                {
                    {RPCResult::Type::OBJ, "", "",
                        {
                            {RPCResult::Type::STR, "receiver", "The receiver qtep address"},
                            {RPCResult::Type::STR, "sender", "The sender qtep address"},
