...
extern std::unique_ptr<QtepState> globalState;
...
inline void initState(){
    ...
    const std::string dirQtep = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<QtepState>(new QtepState(dev::u256(0), QtepState::openDB(dirQtep, hashDB, dev::WithExisting::Trust), dirQtep + "/qtepDB", dev::eth::BaseState::Empty));
...
inline dev::Address createQtepAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline QtepTransaction createQtepTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    QtepTransaction txEth;
    if(recipient == dev::Address()){
        txEth = QtepTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = QtepTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<QtepTransaction> txs){
