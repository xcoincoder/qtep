...
const char * const BITCOIN_CONF_FILENAME = "qtep.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Qtep
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Qtep
    // Mac: ~/Library/Application Support/Qtep
    // Unix: ~/.qtep
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Qtep";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Qtep";
#else
    // Unix
    return pathRet / ".qtep";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Qtep Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Qtep Core Developers";
    }
